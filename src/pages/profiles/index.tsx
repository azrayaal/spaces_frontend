...
import { fetchOtherFollower } from "../../features/otherFollower";
...
export default function DetailUser() {
  ...
  const userDetail = useSelector(
    (state: RootState) => state.OtherUserDetail.userDetail
  );

  const allContent = useSelector((state: RootState) => state.content.data);
  const allOtherFollowing = useSelector(
    (state: RootState) => state.allOtherFollowing.data
  );

  console.log("allFollowing", allOtherFollowing);
  const allOtherFollower = useSelector(
    (state: RootState) => state.allOtherFollower.data
  );

  const filteredContentByuserId = allContent.filter(
    (i) => i.user.id === userDetail.id
  );
  const { postFollow } = useOtherFollow();

  useEffect(() => {
    dispatch(fetchOtherFollowing(dataId));
    dispatch(fetchOtherFollower(dataId));
    dispatch(fetchOtherUserDetail(dataId));
  }, [dataId]);
  ...
  return (
    <>
      ...
      {activeFollowing &&
        allOtherFollowing.map((data: FollowingTypes) => (
          <UserCard
